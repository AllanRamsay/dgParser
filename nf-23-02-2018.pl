...
timeSequence([_AUX,arg(_Type,*(time(Arg0,Arg1,Arg2,Arg3,Arg4)),List)],[TimeSeq|Rest1]):-
	!,
	collectTime(List,[(time(Arg0,Arg1,Arg2,Arg3,Arg4))],TimeSeq,Rest0),
	timeSequence(Rest0,Rest1).
...
collectTime([WORD,arg(_Type,Specifier,List)|OthrArgs],TimeSeq,RefinedTimeSeq2,[WORD,arg(_Type,Specifier,List)|OthrArgs]):-
	\+ length(TimeSeq,1),
	refine(TimeSeq,RefinedTimeSeq),
	refine2(RefinedTimeSeq,RefinedTimeSeq2),
	!.
  
collectTime([WORD,arg(_Type,Specifier,List)|OthrArgs],TimeSeq,RefinedTimeSeq,[WORD,arg(_Type,Specifier,List)|OthrArgs]):-
	refine2(TimeSeq,RefinedTimeSeq),
	!.
  
refine([time(tense(present),_Arg1,_Arg2,_Arg3,_Arg4)|Tail0],Tail1):-
	refine(Tail0,Tail1),
	!.
refine([H|Tail0],[H|Tail1]):-
	refine(Tail0,Tail1),
	!.
refine(X, X) :-
    (var(X); atomic(X)),
...
/**
%% qq(Q::{T0, X}, {T1, ... X ...}) means: add Q :: {T0, X} to the QSTACK and leave {T1 ... X ...} where it is
...
qlf(['?', X0], X1) :-
    !,
    qlf(X0, X1).
qlf(arg(claim, *SPEC, X0),
    claim(opaque(X1))) :-
    !,
    qlf(qq(SPEC, X0), X1).
qlf(arg(xcomp, *SPEC, X0), {xcomp, opaque(qq(SPEC :: {X1, V}, V))}) :-
    !,
    qlf(X0, X1).
%% The template case
...
claim(opaque(qq((existential :: {[time(tense(past))], A})
                 {(qq((existential
                             :: {([eat,
                                    qq((universal
                                         :: {[peach>singular],B}),
  ...
  qq(quantifier :: {RESTRICTOR, X}, {PRED, X}) 
...
extractQQ(opaque(X0), X1, Q, Q) :-
    ...
    fixQuants(X0, X1).
/**
extractQQ(qq([time ... | TIMES], X0), XN, Q0, Q1) :-
  digTheTensesOutOfTheList(..., PATH),
  getTheAspectFromTheLastOne(... perfect))
  getTheQuantifierFromTheFirstOne(..., existential),
  extractQQ(qq(existential :: {PATH, V},
                 qq(existential :: {perfect, V, W}, [X0, W])), XN, Q0, Q1).
...
[qq(existential, ([future, past], X)), qq(existential, (perfect, X, Y)), qq(universal, (peach, C)), qq(existential, (man, D))],
[[eat, dobj(C), subject(D)], Y]

I will be happy when we get that!

You might think I would want
  
[qq(existential, ([future, past], X)), qq(existential, (perfect, X, Y)), qq(universal, (peach, C)), qq(existential, (man, D))],
[eat, Y] &  dobj(Y, C), subject(D)]
**/
